...
\IFRU{Длина в регистре \ECX. \TT{MOVSD} означает \IT{скопировать 32-битное слово}, так что, 16 32-битных слов
это как раз 64 байта.}{The size is in \ECX register. \TT{MOVSD} means \IT{move 32-bit dword}, so, 
...
\lstinputlisting{\IFRU{qr9/5ru.asm}{qr9/5en.asm}}
...
\IFRU{\IT{Почти} то же самое, за исключением иного порядка аргументов в \TT{get\_bit()} и \TT{set\_bit()}.
Перепишем это на Си-подобный код:}
{It is \IT{almost} the same, except of different order of arguments of \TT{get\_bit()} and \TT{set\_bit()}. 
