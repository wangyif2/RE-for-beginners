...
значение из памяти, через шину с памятью, кеширование, итд.}
{is assembler macro, aligning next label so that it will be stored at address aligned by 4 bytes (or 16). 
This is very suitable for processor, because it can fetch 32-bit value from memory through bus, 
through cache memory, etc, in much effective way if it is aligned.}
