...
\IFRU{Итак, индексация массива это просто \IT{массив[индекс]}. 
...
{So, array indexing is just \IT{array[index]}.
...
\lstinputlisting{\IFRU{arrays/13_stack_ru.lst}{arrays/13_stack_en.lst}}

\IFRU{Команда \TT{a[19]=чего\_нибудь} записывает последний \Tint в пределах массива (пока что!)}
{Instruction \TT{a[19]=something} writes last \Tint in array bounds (yet!)}
