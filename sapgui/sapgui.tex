...
\newcommand{\TDWNC}{TDW\_NOCOMPRESS}
...
функциями MFC, затем вызывается atoi()\footnote{Стандартная функция Си, конвертирующая число в строке в число}.
...
atoi()\footnote{standard C library function, coverting number in string into number} is called. 
...
\IFRU{Да}{Yes}.\IFRU{Функция} \href{http://msdn.microsoft.com/en-us/library/tb2sfw2z(VS.80).aspx}{getenv\_s()} \IFRU{это \IT{безопасная} версия функции getenv()\footnote{Стандартная функция Си возвращающая значение переменной окружения} в MSVC}
{function is Microsoft security-enhanced version of getenv()\footnote{Standard C library returning environment variable}}.
